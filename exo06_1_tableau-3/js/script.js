...
btnTrie2.addEventListener('click', function() {
    // Copie du tableau
    const tabSort = [...tableau];
    ...
    for(let i = 1; i < tabSort.length; i++) {
        ...
        if(tabSort[i] < tabSort[i-1]) {
            ...
            // - On retire la valeur (splice)
            tabSort.splice(i, 1);
            // - On l'insert à la position
    ...
    displayResult.innerHTML = `Trie par insertion ${tabSort.join(' > ')} en ${nbOperation} opérations.`;
});
